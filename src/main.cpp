...
#pragma endregion
    ...
    if (argc != 5 && argc != 4)
    {
        std::cout << "\n\t esperando:\t <x> <y> <z> <seed:optional>\n"
                  << "Recibidos " << argc << " Argumentos" << std::endl;
        return 1;
    }
    ...
    if (argc == 5)
    {
        ...
        Wfc3D result = Wfc3D(patterns, weights, {x, y, z}, seed);
        display_scene_from_matrix(result);
    }
    if (argc == 4)
    {
        int x = std::stoi(argv[1]);
        int y = std::stoi(argv[2]);
        int z = std::stoi(argv[3]);
        std::cout << "Generando 3D con dimensiones: " << x << "x" << y << "x" << z << std::endl;
        Wfc3D result = Wfc3D(patterns, weights, {x, y, z});
        display_scene_from_matrix(result);
    }
